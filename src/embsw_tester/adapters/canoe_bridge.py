...
class CanoeBridgeTransport(Protocol):
    def execute(
        self,
        command_type: str,
...
class CanoeBridgeRequest:
    ...
    @classmethod
    def from_dict(cls, payload: Mapping[str, Any]) -> "CanoeBridgeRequest":
        return cls(
...
class CanoeBridgeResponse:
    ...
    @classmethod
    def from_dict(cls, payload: Mapping[str, Any]) -> "CanoeBridgeResponse":
        return cls(
            request_id=str(payload["request_id"]),
...
class JsonLineCanoeBridgeTransport:
    def __init__(
        self,
        process: Any,
    ...
    def execute(
        ...
        try:
            ...
            return _failed_bridge_result(f"CANoe bridge transport failed: {exc}.")

        if response.request_id != request.request_id:
            return _failed_bridge_result(
                "CANoe bridge returned unexpected response id "
                f"'{response.request_id}' for request '{request.request_id}'."
            )
        return response.to_adapter_result()

    def _write_request(self, request: CanoeBridgeRequest) -> None:
    ...
    def _read_response(self) -> CanoeBridgeResponse:
        ...
        return CanoeBridgeResponse.from_dict(payload)


def create_canoe_bridge_process_transport(
    ...
    return JsonLineCanoeBridgeTransport(
        process,
