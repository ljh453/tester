...
def test_runtime_delay_sleeps_in_pollable_slices(tmp_path: Path):
    test_file = tmp_path / "pollable-delay.yaml"
    test_file.write_text(
        """
testcases:
  - name: pollable_delay_case
    steps:
      - delay:
          ms: 120
""".strip(),
    ...
    result = run_package(
        compile_file(test_file),
        run_id="pollable-delay-run",
        sleep_fn=sleep_calls.append,
        event_callback=streamed_events.append,
        run_control=RuntimeControl(poll_interval_s=0.05),
    )

    running_delay_events = [
        event
        for event in streamed_events
        if event.command_type == "delay" and event.status == "running"
    ]
    assert result.status == "passed"
    assert len(sleep_calls) > 1
    assert max(sleep_calls) <= 0.05
    assert round(sum(sleep_calls), 3) == 0.12
    assert len(running_delay_events) > 1
    assert any("remaining_ms" in event.outputs for event in running_delay_events)


def test_runtime_delay_honors_pause_request_during_delay(tmp_path: Path):
    test_file = tmp_path / "pause-during-delay.yaml"
    test_file.write_text(
        """
testcases:
  - name: pause_during_delay_case
    steps:
      - delay:
          ms: 120
""".strip(),
    ...
    def sleep_and_pause(seconds: float) -> None:
    ...
    result = run_package(
        compile_file(test_file),
        run_id="pause-during-delay-run",
        sleep_fn=sleep_and_pause,
        event_callback=resume_on_pause,
        run_control=RuntimeControl(
            control_file=control_file,
    ...
    paused_event = next(event for event in streamed_events if event.status == "paused")
    assert result.status == "passed"
    assert paused_event.command_type == "delay"
    assert paused_event.outputs["reason"] == "pause_requested"
